	...
	while (cur)
	{
		...
		else
		{
			if (!last)
				...
				cur->group = get_gp_4(ps->order[layer - 1], ps->sym_p_pt * 4, index);
		}
		cur = cur->next;
	}
}
...
void	set_order_4(char **order, int layers, int max_sym)
{
	...
	order[layers - 1] = ft_strnew(max_sym);
	// ft_strncpy(order[layers - 1], ((layers % 2) ? pat_4[1] : pat_4[0]), max_sym);
	ft_strncpy(order[layers - 1], pat_4[1], max_sym);
	...
	while (i >= 0)
	{
		...
		while ((j < num_elements))
		{
			// if (i % 2 == 0) // start with 0
			// {
			ft_strncpy((*(order + i) + j), pat_4[0], max_sym); // 0011
			// if (order[i + 1][j / max_sym] == '0')
			// 	ft_strncpy((*(order + i) + j), pat_4[0], max_sym); // 0011
			// else
			// 	ft_strncpy((*(order + i) + j), pat_4[1], max_sym); // 1100
			// }
			// else
			// {
			// 	if (order[i + 1][j / max_sym] == '0')
			// 		ft_strncpy((*(order + i) + j), pat_4[2], max_sym); // 1100
			// 	else
			// 		ft_strncpy((*(order + i) + j), pat_4[3], max_sym); // 1100
			// }
			j += max_sym;
		}
		i--;
